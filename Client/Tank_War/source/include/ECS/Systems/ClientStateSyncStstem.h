...
private:
	void SyncTanks(uint64_t server_timestamp, std::vector<TankState>& tankStates, std::unordered_map<SyncID, Entity>& clienttankmap);
	void SyncBullet(uint64_t server_timestamp, std::vector<BulletState>& bulletStates, std::unordered_map<SyncID, Entity>& clientbulletmap);
	void SyncWall(std::vector<WallState>& wallStates, std::unordered_map<SyncID, Entity>& clientwallmap);
	void SyncProp(std::vector<PropState>& propStates, std::unordered_map<SyncID, Entity>& clientpropmap);

	void handleSyncTankFromServer(uint64_t server_timestamp, TankState& state, Entity entity);
	void handleCreateClientTank(uint64_t server_timestamp, TankState& state);

	void handleSyncBulletFromServer(uint64_t server_timestamp, BulletState& state, Entity entity);
	void handleCreateClientBullet(uint64_t server_timestamp, BulletState& state);

	void handleSyncWallFromServer(WallState& state, Entity entity);
	void handleCreateClientWall(WallState& state);

	void handleSyncPropFromServer(PropState& state, Entity entity);
	void handleCreateClientProp(PropState& state);
