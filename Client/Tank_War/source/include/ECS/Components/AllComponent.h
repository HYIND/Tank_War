...
// ================ 渲染组件 ================
#include "Sprite.h"             // 精灵渲染
#include "GIFAnimator.h"        // GIF动画
...
// ================ 在线游戏 ================
#include "Interpolation.h"		// 位置插值组件
...
constexpr const char* GetComponentName() {
	...
	else if constexpr (std::is_same_v<T, Sprite>) return "Sprite";
	...
	else if constexpr (std::is_same_v<T, GIFAnimator>) return "GIFAnimator";
	else return "Unknown";
}