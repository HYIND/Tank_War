...
Task<void> GameService::GameOver(const GameID& gameId)
{
	...
	std::shared_ptr<GameInstance> gameInstance;
	{
		...
		if (!_GameIdToGameInstance.Exist(gameId))
			co_return;
		gameInstance = _GameIdToGameInstance[gameId];
	...
	auto players = gameInstance->GetAllPlayerIds();
...
Task<void> GameService::OnStub_ProcessNewGame(json& js_src, json& js_dest)
{
	...
	try
	{
		...
		std::cout << std::format("New game created: {}, players: {}\n", gameId, players.size());
...
Task<void> GameService::ProcessPlayerJoin(const JsonProtocolSession& session, json& js_src, json& js_dest)
{
	...
	try
	{
		...
		std::shared_ptr<GameInstance> gameinstance;
		if (!_GameIdToGameInstance.Find(gameid, gameinstance))
		{
			js_dest["result"] = -1;
